...
% *****************************************************************  
% filename: wb_LDPC_Decoder_MR_hard.m
% This code is for Majority-Rule based message passing decoding.
% *****************************************************************
% --------------------- Input --------------------- 
...
function [LR_p] = wb_LDPC_Decoder_MR_hard(H,Q1,Q2,Num_iteration,LR_f)
...
for iteration = 1:Num_iteration
    if iteration==1
        ...
        for t=1:n
            ...
            for m=1:j
                if (Q1(m,t) ~=0) % some element of Q1 matrix is 0 due to lack of link
                    temp_LR_r = sum_LR_r - mod(LR_r(t,Q1(m,t))+LR_f(t),2);
                    if t<= L
                        if temp_LR_r >= (k-1)/2 + 1
